...
fn vec_mul(a: vec2f, b: vec2f) -> vec2f {
...
fn premulDepressedCubic_findRoots_fast(c0divn2: f32, c1divn3: f32) -> vec3f {
  ...
  if !isFinite(d) {
    ...
    if 0.0 < d {
      ...
      //let u = rcbrt q in (u * fma(q, u, c1divn3), MAX_F32, MAX_F32)
    }
    else {
      ...
      if !isFinite(u) {
        ...
        //(cbrt (c0divn2 + c0divn2), MAX_F32, MAX_F32) // Even in f32 with subnormal-flushing and no fma, abs(exact answer) in this codepath is always < 2 ** -20.6, so we just round to 0.
...
fn quartic_findRoots(c0: f32, c1: f32, c2: f32, c3: f32, c4: f32,) -> vec4f {
  ...
  if !(isFinite(m.x) && isFinite(m.y) && isFinite(m.z) && isFinite(m.w)) {
    ...
    return vec4f(t.x + m3divn4, t.y + m3divn4, t.z + m3divn4, t.w + m3divn4);
  }
}
...
fn mulAdd(a: vec2f, b: vec2f, c: vec2f) -> vec2f {
...
fn twoCirclesIntersect(c0_center: vec2f, c0_radius: f32, c1_center: vec2f, c1_radius: f32) -> array<vec2f, 2> {
  ...
  return array(mulAdd(z, centerDiff, c0_center), mulAdd(conj(z), centerDiff, c0_center),);
}
...
fn circleLineIntersect(center: vec2f, radius: f32, normal: vec2f, shift: f32) -> array<vec2f, 2> {
  ...
  return array(mulAdd(z, normal, center), mulAdd(conj(z), normal, center),);
}
...
fn twoLinesIntersect(a_normal: vec2f, a_shift: f32, b_normal: vec2f, b_shift: f32) -> vec2f {
  return vec_mul(a_normal, vec2f(a_shift, halfPlaneToLineFunc(vec_mul(b_normal, conj(a_normal)), b_shift, a_shift,)));
}

@group(0) @binding(0)
var<storage, read> shapes: array<f32>;

// The size of this must be triangle_count(array_length(shape_idx))
@group(0) @binding(1)
...
// This is a tiny array, so we just allocate the maximum number of intersections we currently support (256).
// A production implementation would divide these into 64 element chunks.
var<workgroup> sh_offsets: array<u32, 256>;
var<workgroup> sh_points: array<vec2f, 256>;

@compute @workgroup_size(1)
fn implied_points(@builtin(global_invocation_id) id: vec3<u32>) {
  let idx = unrank(id.x);
  var intersections = num_intersections(idx);

  // Assemble a prefix sum of intersection counts
  sh_offsets[id.x] = intersections;
  for (var i = 0u; i < firstTrailingBit(config.count); i++) {
    workgroupBarrier();
    if id.x >= (1u << i) {
      intersections += sh_offsets[id.x - (1u << i)];
    }
    workgroupBarrier();
    sh_offsets[id.x] = intersections;
  }
  ...
  if idx.x == idx.y {
    ...
    if v == SHAPE_BEZIER {
      ...
      sh_points[intersections + 0] = vec2f(shapes[i + 1], shapes[i + 2]);
      sh_points[intersections + 1] = vec2f(shapes[i + 5], shapes[i + 6]);
    }
  ...
  switch (r << 3) | l {
    ...
    case (SHAPE_BEZIER << 3) | SHAPE_BEZIER: {
      ...
      sh_points[intersections + 0] = pt[0];
      sh_points[intersections + 1] = pt[1];
      sh_points[intersections + 2] = pt[2];
      sh_points[intersections + 3] = pt[3];
    }
    case (SHAPE_BEZIER << 3) | SHAPE_CIRCLE, (SHAPE_CIRCLE << 3) | SHAPE_BEZIER: {
      // TODO
